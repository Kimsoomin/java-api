...
public class RegExp1 {
	public static void main(String[] args) {
		...
		String[] pattern = {
				...
				"c[a-zA-Z]",// c로 시작하는 2글자 영단어(대소문자 관계없음)
				"c[a-zA-Z0-9]",	// c로 시작하고 숫자와 영어로 조합된 2글자
				"c\\w",		// c로 시작하고 숫자와 영어로 조합된 2글자(위와 동일)
				"c.",		// c로 시작하는 2글자(기호 포함)
				"c.*",		// c로 시작하는 모든 문자열(기호 포함)
				"c\\.",		// escape 문자로 글자 그대로
				"c\\d",		// c와 숫자로 구성된 두자리 문자열. demical 10진수
				"c[0-9]",	// c 와 숫자로 구성된 두자리 문자열(위와 동일)
				"c.*t",		// c로 시작하고 t로 끝나는 모든 문자열
				"[b|c].*",	// b 또는 c 로 시작하는 문자열
				"[bc].*",	// b 또는 c 로 시작하는 문자열(위와 동일)
				"[b-c].*",	// b 또는 c 로 시작하는 문자열(위와 동일)
				".*a.*",	// a를 포함하는 모든 문자열  * : 0또는 그 이상의 문자, null포함
				".*a.+",	// a를 포함하는 모든 문자열	 + : 1또는 그 이상의 문자, null허용하지 않음
			// 위에 반드시 한개 이상의 문자가 와야 하므로 a 로 끝나는 단어는 제외
				"[b|c].{2}",// b또는 c로 시작하는 세자리 문자열
				"[^b|c].*"	// b또는 c로 시작하는 않는 문자열
		};
		for (int i = 0; i < pattern.length; i++) {
			Pattern p = Pattern.compile(pattern[i]);	// Shallow copy 얖은 카피
			System.out.println();
